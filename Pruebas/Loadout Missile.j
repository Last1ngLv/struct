...
*************************************************************************************/
    globals
        ...
        private constant integer DEFAULT_CHOICE_ABILITY = 'AM05'
        ...
        private constant string  DEFAULT_MODEL_PATH = "Miss\\Shot II Blue.mdx"
        ...
        private constant real    DEFAULT_LEAP_DUMMY_SCALE = 1.50
...
library LoadoutOrbBalance
    globals
        ...
        constant real LOADOUT_ORB_WIND_BASE_AOE = 100.
...
library LoadoutManaSwapNew initializer Init uses Table, SpellIndex, LoadoutOrbBalance
//******************************************************************************
// Simple prototype:
// 1) Give helper mana ability.
// 2) When orb spell event triggers: remove helper, restore mana, set orb level to CD_LEVEL.
// 3) After timer: restore previous orb level.
//******************************************************************************
    globals
        ...
        private constant integer ABILITY_RAY = 'AM01'
        private constant integer ABILITY_FIRE = 'AM02'
        private constant integer ABILITY_POISON = 'AM03'
        private constant integer ABILITY_WIND = 'AM04'
        private constant integer ABILITY_DARK = 'AM05'
        private constant integer ABILITY_BLOOD = 'AM06'

        private constant integer MANA_ABILITY = 'A00Y'

        private constant real ABILITY_CD_LEVEL1 = 5.00
        private constant real ABILITY_CD_STEP = -1.00
    ...
    private function OnCooldownFinish takes nothing returns nothing
        ...
        if GetUnitTypeId(u) != 0 then
            call UnitRemoveAbility(u, MANA_ABILITY)
    ...
    function TriggerLoadoutManaSwapForAbility takes unit u, integer abilityRaw returns nothing
        call BJDebugMsg("Faz0") 
        if abilityRaw == ABILITY_RAY then
            call BJDebugMsg("Faz1") 
    ...
    function LoadoutMana takes unit u, integer abilityRaw returns nothing
        call BJDebugMsg("Fazz") 
        call TriggerLoadoutManaSwapForAbility(u, abilityRaw)
    endfunction

    private function Init takes nothing returns nothing
        set byUnit = Table.create()
        call RegisterSpellEffectEvent(ABILITY_RAY, function FunctionEventRay)
        call RegisterSpellEffectEvent(ABILITY_FIRE, function FunctionEventFire)
        call RegisterSpellEffectEvent(ABILITY_POISON, function FunctionEventPoison)
        call RegisterSpellEffectEvent(ABILITY_WIND, function FunctionEventWind)
        call RegisterSpellEffectEvent(ABILITY_DARK, function FunctionEventDark)
        call RegisterSpellEffectEvent(ABILITY_BLOOD, function FunctionEventBlood)
    endfunction
endlibrary

library LoadoutMissile initializer Init uses SpellIndex, Missile, PlayerMissileLoadout, IsUnitChanneling, DamageTextUtil, LoadoutOrbBalance, LoadoutManaSwapNew /* v2.0
*************************************************************************************
*
...
//* ==============
    ...
    private function FireMissile takes SpellIndex dex returns nothing
        ...
        if bonusActive[missile] then
            call LoadoutMana(source, chosen)
        endif
