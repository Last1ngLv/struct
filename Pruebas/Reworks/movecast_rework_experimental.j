...
globals
    ...
    private constant boolean MOVECAST_TRACE = true
...
struct MovementData
    ...
    //=======================================================================
    method traceSessionExit takes string msg returns nothing
        if (not MOVECAST_TRACE) then
            return
        endif
        call BJDebugMsg("[MoveCast] " + msg)
    endmethod
    ...
    private static method onRecastIssue takes nothing returns nothing
        ...
        if not .hasTimedCastSession() then
            return
        endif
        if not .hasDummy and .isMoving then
    ...
    //=======================================================================
    private method queueTrackedCastReissue takes nothing returns nothing
        if .recastIssueTim != null then
            call ReleaseTimer(.recastIssueTim)
            set .recastIssueTim = null
        endif
        set .recastIssueTim = NewTimerEx(this)
        call TimerStart(.recastIssueTim, 0.00, false, function thistype.onRecastIssue)
    endmethod
    ...
    private static method onCastSessionTick takes nothing returns nothing
        ...
        if (.source == null) or (GetUnitTypeId(.source) == 0) or (not UnitAlive(.source)) then
            ...
            call ReleaseTimer(t)
            set t = null
            call .traceSessionExit("MC:tick-destroy")
            call .destroy()
            return
        endif
        call .updateTrackedCastTarget()
        ...
        if .castTimeRemaining <= 0. then
            ...
            call ReleaseTimer(t)
            set t = null
            call .traceSessionExit("MC:tick-timeout")
    ...
    //=======================================================================
    private method lockSessionInput takes nothing returns nothing
        set .sessionInputLocked = true
    endmethod

    //=======================================================================
    private method unlockSessionInput takes nothing returns nothing
        set .sessionInputLocked = false
    endmethod
    ...
    private static method onIdleExpire takes nothing returns nothing
        ...
        if (.source == null) or (GetUnitTypeId(.source) == 0) or (not UnitAlive(.source)) then
            call .traceSessionExit("MC:idle-source")
            call .destroy()
            return
        endif
        if GetUnitAbilityLevel(.source, IGNORE_BUFF_ID) > 0 then
            call .traceSessionExit("MC:idle-ignore")
            call .destroy()
            return
        endif
        ...
        if not .isMoving then
            call .traceSessionExit("MC:idle-stop")
            call .destroy()
            return
        endif
        ...
        if .followTarget then
            if (.targetUnit == null) or (GetUnitTypeId(.targetUnit) == 0) or (not UnitAlive(.targetUnit)) then
                call .traceSessionExit("MC:idle-follow0")
                call .destroy()
                return
            endif
            call .armIdleCleanup()
        ...
        if .getDistanceSquared(sx, sy, .targetX, .targetY) <= ARRIVAL_THRESHOLD_SQ then
            call .traceSessionExit("MC:idle-arrive")
            call .destroy()
            return
        endif
    ...
    private static method periodic takes nothing returns nothing
        ...
        if (.source == null) or (GetUnitTypeId(.source) == 0) or (not UnitAlive(.source)) then
            call .traceSessionExit("MC:per-source")
            call .destroy()
            return
        endif
        if GetUnitAbilityLevel(.source, IGNORE_BUFF_ID) > 0 then
            call .traceSessionExit("MC:per-ignore")
            call .destroy()
            return
        endif
        ...
        if .followTarget then
            if (.targetUnit == null) or (GetUnitTypeId(.targetUnit) == 0) or (not UnitAlive(.targetUnit)) then
                ...
                call .traceSessionExit("MC:per-follow0")
                call .destroy()
                return
            endif
        ...
        if .dummy != null and GetUnitTypeId(.dummy) != 0 then
            ...
            if not IsTerrainWalkable(dummyX, dummyY) then
                ...
                set .followTarget = false
                call .stopDummyMovement()
                call .traceSessionExit("MC:per-path")
                call .destroy()
                return
            endif
            ...
            if (not .followTarget) and (distSq < ARRIVAL_THRESHOLD_SQ) then
                ...
                if not IsTerrainWalkable(dummyX, dummyY) then
                  ...
                  set .targetUnit = null
                  call .stopDummyMovement()
                  call .traceSessionExit("MC:per-arrive")
                  call .destroy()
              endif
          else
              set .isMoving = false
              ...
              set .followTarget = false
              call .stopDummyMovement()
              call .traceSessionExit("MC:per-dummy0")
              call .destroy()
          endif
      endmethod
    ...
    method onSpellEffect takes nothing returns nothing
        if .channelOrderId != 0 then
            if .isMoving and (not .hasDummy) then
                call .startDummyMovement()
            endif
            call .lockSessionInput()
            call .armIdleCleanup()
        endif
    endmethod
    ...
    if abilId == 'AM01' or abilId == 'AM02' or abilId == 'AM03' or abilId == 'AM04' or abilId == 'AM05' or abilId == 'AM06' then
      ...
      if MovementData.has(u) then
          ...
          if abilityOrderSpells.has(abilId) and (data.channelOrderId != 0) and (data.castAbilityId == abilId) then
              ...
              call data.traceSessionExit("MC:endcast-end")
...
private function OnStopOrder takes nothing returns boolean
    ...
    if orderId == ORDER_ID_STOP or orderId == ORDER_ID_HOLD_POSITION then // stop, holdposition
          if MovementData.has(u) then
              ...
              call data.traceSessionExit("MC:stop-destroy")
              call data.destroy()
          endif
      endif
