...
/*
    ============================================================================
    EquipmentCore (version documentada)
    ============================================================================
    OBJETIVO DEL SISTEMA
    - Renderizar y controlar la UI de equipo por jugador/unidad.
    - Conectar slots de equipo con InventoryCore.
    - Aplicar validaciones de equipamiento y ejecutar eventos onEquip/onUnequip.

    MAPA MENTAL RAPIDO
    1) create(unit) -> crea/reusa widgets UI y enlaza la unidad con un struct Equipment.
    2) show(flag, cam) -> muestra/oculta panel + mantiene una sola UI activa por jugador.
    3) LClick/RClick en slots -> equipar, desequipar, transferir entre inventario y equipo.
    4) equip/unequip -> reglas + efectos (abilities/bonuses/eventos + iconos UI).

    CONVENCIONES IMPORTANTES
    - "this.user.id * MAX_SLOTS + i" se usa como indice lineal por jugador.
      Ejemplo:
        jugador 0 -> indices [0..13]
        jugador 1 -> indices [14..27]
    - this.item[i] guarda el InvItem equipado en el slot i.
    - this.itemId[i] guarda custom id asociado (instancia/stack/variacion de item).

    NOTA DE COMPILACION
    - Esta copia mantiene los mismos identificadores del original:
      library EquipmentCore / struct Equipment.
    - No la cargues junto al EquipmentCore original al mismo tiempo.
      Usala como archivo de estudio o renombra simbolos si la quieres activar.
*/
    
    globals
        // Filtros callback para botones de slots de equipo (L/R click).
        // Se asignan en Init() y luego se inyectan en create().
        public filterfunc FuncLClickSlot = null
        public filterfunc FuncRClickSlot = null

        // "slot seleccionado previamente" por jugador.
        // Se usa para flujos de intercambio inventario <-> equipo.
        public integer array PlayerLastSlot 
    endglobals
    
    // Texto del titulo del panel de equipo.
    // Puedes cambiar esta funcion para mostrar nombre custom de hero o clase.
    ...
    struct Equipment
        ...
        // Widgets visuales cacheados por jugador (indice lineal por jugador/slot).
        ...
        /*
            create(u)
            - Vincula unidad + jugador + user struct.
            - Crea una sola vez los botones/marcos por jugador.
            - Siempre crea el charModel para esta instancia de unidad.
        */
        static method create takes unit u returns thistype
            ...
            // Si no existe el primer boton, asumimos que la UI base del jugador aun no existe.
            if (this.getButton(0) == 0) then
                ...
                // Config comun para los 14 slots.
                loop
                    exitwhen i == 14
                    ...
                    // Click handlers de cada slot (inyectados desde Init()).
                ...
                // Marco principal del panel de equipo.
            ...
            // Modelo del personaje (siempre por instancia de Equipment, no cacheado global).
            ...
            // Titulo por jugador (texto local del owner).
        ...
        /*
            equip(itm, equipSlot)
            Flujo:
            1) Validaciones de slot/tipo/requisitos.
            2) Validacion custom via itm.equipCondition (TriggerEvaluate).
            3) Aplica ability, registra item, actualiza icono slot.
            4) Dispara evento onEquip y aplica bonuses.

            EJEMPLO GUIADO:
            - itm.slot = 3 (tercer slot base) y equipSlot = 2
            - si pasa validaciones:
                this.item[2]   = itm
                this.itemId[2] = itm.tempCustomId
                slotButton[2] muestra icono del item
                onEquip (si existe y evalua true) se ejecuta
                itm.applyBonuses(this.unit)
        */
        method equip takes InvItem itm, integer equipSlot returns boolean
            ...
            // Regla custom por item (scriptable).
        ...
        /*
            unequip(itm, slot)
            - Quita ability asociada.
            - Ejecuta evento onUnequip (si aplica).
            - Limpia this.item[slot], icono del slot y bonuses.
        */
        method unequip takes InvItem itm, integer slot returns boolean
            ...
            // Evento de salida de item equipado.
            set InvEventPlayer = this.player
        ...
        // Limpieza de instancia (lista doblemente enlazada + charModel).
        ...
        /*
            onDisplay
            Timer de refresco visual cuando la UI de equipo esta visible.
            Mantiene la camara y actualiza interfaz solo para jugador local.
        */
        ...
        /*
            show(flag, cam)
            - Muestra/oculta UI de equipo de esta instancia.
            - Asegura una UI de equipo activa por jugador.
            - Si inventario de otra unidad esta abierto, lo cierra.
        */
        method show takes boolean flag, Camera cam returns nothing
            ...
            if (flag) then
                ...
                // Timer de refresco compartido para todas las UIs visibles.
                ...
                // Cierra otra UI de equipo previa del mismo jugador.
                ...
                // Evita conflicto con inventario de otra unidad.
            ...
            // TODO heredado del autor original (no afecta funcionamiento).
    ...
    /*
        ...
        Click izquierdo en slot de inventario mientras panel de equipo esta activo.
        Uso principal:
        - Si ya habia slot de equipo "armado", intenta mover item entre inventario/equipo.
        - Si no, solo mueve selector visual al slot candidato.
    */
    ...
    /*
        ...
        Click derecho en inventario:
        - Intenta equipar inmediatamente el item.
        - Si habia item equipado en ese slot, lo mueve de vuelta al inventario.
        - Si falla el equip, restaura estado previo.
    */
    ...
    /*
        RClickItemSlot
        Click derecho sobre slot de equipo:
        - Desequipa item y lo manda al inventario.
        - Si inventario esta lleno, revierte (re-equipa).
    */
    ...
    /*
        LClickItemSlot
        Click izquierdo sobre slot de equipo:
        - Selecciona visualmente slot actual.
        - Si hay item seleccionado en inventario compatible, intenta equiparlo.
        - Actualiza tooltip del item del slot.
    */
    ...
    /*
        Init
        Enlaza handlers locales de Equipment con InventoryCore.
        Sin esto, los botones se ven pero no tendran comportamiento.
    */
